...
function submitSiteSearch(frm, page) {
    ...
    if (qryParam.indexOf("sellertype") > -1 && $(frm).find('input[name="sellertype"]').val() > 0) {
        urlString = urlString + setQueryParamSeperator(urlString) + 'sellertype-' + $(frm).find('input[name="sellertype"]').val();
    }
...
function styleShopPhoneNumberFld(element = "input[name='shop_phone']", destroy = false) {
    var inputList = document.querySelectorAll(element);
    var country = '' == langLbl.defaultCountryCode ? 'in' : langLbl.defaultCountryCode;
    inputList.forEach(function (input) {
        if (true == destroy) {
...
function displaySubscriptionExpirePopup() {
    fcom.ajax(fcom.makeUrl('seller', 'SubscriptionExpirePopup'), '', function (t) {
        fcom.updateFaceboxContent(t, 'faceboxWidth small-fb-width');
    });
}


$(document).on('click', '.rfq-btn', function () {
    $.facebox(function () {
        fcom.updateFaceboxContent('RFQ Module Is Under Progress', 'faceboxWidth collection-ui-popup small-fb-width rfq-info-popup');
        fcom.resetFaceboxHeight();
    });
});

$(document).on('click', '.toggle-languages--js', function (event) {
    event.stopPropagation();
    $('.join-free--js').removeClass('is-active');
    $('.language-setting--js').toggleClass('is-active');
    $('#notificationList-header').html('');
});
$(document).on('click', '.toggle-signup--js', function (event) {
    event.stopPropagation();
    $('.language-setting--js').removeClass('is-active');
    $('.join-free--js').toggleClass('is-active');
    $('#notificationList-header').html('');
});
...
$(document).on('click', '.language-setting--js', function (event) {
    event.stopPropagation();
    $('#notificationList-header').html('');
});
$(document).on('click', '.join-free--js', function (event) {
    event.stopPropagation();
    $('#notificationList-header').html('');
});

$(document).on('click, mouseover', '.header__nav-settings', function (event) {
    $('.header__actions-group .dropdown').removeClass('show');
    $('.header__actions-group .dropdown-menu').removeClass('show');
});

$(document).on('click', '#notificationList-header', function (event) {
    event.stopPropagation();
});
