...
function setSelectedSellerTypeValue(id) {
    if (id ==='' || id == undefined) {
        id = $('input[name="sellertype"]').val();
    }
    
    var currentId = 'seller-type--js-' + id;
    var e = document.getElementById(currentId);
    if (e != undefined) {
        var catName = e.text;
        $(e).parent().siblings().removeClass('is-active');
        $(e).parent().addClass('is-active');
        $('#selected_seller_type_value--js').html(catName);
        $('#selected_seller_type_value--js').closest('form').find('input[name="sellertype"]').val(id);
        $('.dropdown__trigger-js').parent('.dropdown').removeClass("is-active");
    }
}
